...
const Login = ({ onClose }) => {
    ...
    const handleLogin = (e) => {
        e.preventDefault();
        const foundUser = user.find(u => u.name === userName.trim().toLowerCase() && u.pass === password.trim())
        if (!foundUser) {
            alert("invalid user name or password")
            return
        }
        if (foundUser.name === "you") {
            navigate("/mainPage")
                ...
                <form
                    ...
                    <div className='flex flex-row items-center justify-between gap-6'>
                        <button
                            type="button"
                            onClick={onClose}
                            className='text-[#F99147] border border-[#F99147] rounded px-2 py-1 hover:bg-[#F99147]/60 '
                        >
                            Cancel
                        </button>
