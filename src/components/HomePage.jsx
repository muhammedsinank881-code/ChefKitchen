  ...
  return (
    <div className="h-screen bg-[#1f2433] text-white p-3 sm:p-6 md:pl-10 flex flex-col">
      ...
      <div className="flex items-center justify-between font-barlow p">
        ...
        <div className="relative">
          <input
            ...
            className="w-full sm:w-72 rounded-xl bg-[#2a2f42] border border-[#343a52] pl-10 pr-4 py-2 text-sm"
          />
      ...
      <div className="overflow-x-auto no-scrollbar">
        <nav className="flex mt-6 border-b border-[#2f354a] gap-6">
          {[
            ['today', 'Today Special'],
            ['special', 'Our Special'],
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 bg-[#2a2f42] border border-[#343a52] px-4 py-2 rounded-xl text-sm text-gray-300"
          >
            {orderType}
            <svg
          ...
          {isOpen && (
            <div className="absolute right-0 mt-2 w-40 bg-[#2a2f42] border border-[#343a52] rounded-xl shadow-lg z-10">
        ...
        >
          {visibleDishes.length === 0 ? (
            ...
            visibleDishes.map(item => (
              <div key={item.id} className="relative rounded-2xl p-4 text-center bg-[#2a2f42]">
                {isDishInCart(item.id) && (
                  <span className="absolute top-5 right-5 w-3 h-3 rounded-full bg-green-400" />
                )}

                <img src={item.img} className="mx-auto h-36 sm:h-44 object-contain" />
                ...
                <button
                  ...
                  className={`mt-3 w-full py-2 rounded-lg font-semibold transition active:scale-97 ${item.bowls ? 'bg-orange-500 hover:opacity-90' : 'bg-gray-600 cursor-not-allowed'
                    }`}
                >
